...
std::pair<typename Tree<Key_T, T>::iterator, bool> Tree<Key_T, T>::insert(
...
template <typename Key_T, typename T>
bool Tree<Key_T, T>::insert(node_*& root, const_key_type& key,
  ...
  if (!root)
    ...
    if (flag) balancerInsert(root, key);
  }
  return flag;
}
...
void Tree<Key_T, T>::clear(node_*& root) {
  if (root) {
    if (root->right_) clear(root->right_);
    if (root->left_) clear(root->left_);
    removeNode(root);
  }
}

template <typename Key_T, typename T>
typename Tree<Key_T, T>::node_* Tree<Key_T, T>::find_(
    node_* root, const_key_type& key) const {
  if (!root) return nullptr;
  if (key == root->key_) return root;
  if (key < root->key_) return find_(root->left_, key);
  return find_(root->right_, key);
}
template <typename Key_T, typename T>
typename Tree<Key_T, T>::node_* Tree<Key_T, T>::find__(
    node_* root, const_value_type& value) const {
  if (!root) return nullptr;
  if (value == root->data_) return root;
  node_* tmp = nullptr;
  tmp = find__(root->left_, value);
  if (!tmp) tmp = find__(root->right_, value);
  return tmp;
}
template <typename Key_T, typename T>
inline typename Tree<Key_T, T>::iterator Tree<Key_T, T>::find(
    const_key_type& key) const {
...
Tree<Key_T, T>::Tree(
  ...
  for (const auto& i : items) insert(i.first, i.second);
}
...
template <typename Key_T, typename T>
Stack<typename Tree<Key_T, T>::key_type>& Tree<Key_T, T>::getTree(
    const node_* node, Stack<key_type>& stack) const {
  if (node) {
...
template <typename Key_T, typename T>
std::pair<Stack<typename Tree<Key_T, T>::key_type>,
          Stack<typename Tree<Key_T, T>::value_type>>&
Tree<Key_T, T>::getTree(
    const node_* node,
...
template <typename Key_T, typename T>
typename Tree<Key_T, T>::node_* Tree<Key_T, T>::remove(node_*& root,
                                                       const_key_type& key) {
  if (!root) return root;
  if (key < root->key_)
...
template <typename Key_T, typename T>
inline typename Tree<Key_T, T>::size_type Tree<Key_T, T>::max_size()
    const noexcept {
  return std::numeric_limits<size_type>::max() / sizeof(Tree<Key_T, T>::node_);
}

template <typename Key_T, typename T>
inline typename Tree<Key_T, T>::node_* Tree<Key_T, T>::getParent(
    const_key_type& key) const {
  node_* tmp = find_(key);
  if (tmp) return find_(key)->parent_;
  return nullptr;
}
template <typename Key_T, typename T>
typename Tree<Key_T, T>::key_type Tree<Key_T, T>::getParentData(
    const_key_type key) const {
  node_* tmp = getParent(key);
  if (!tmp) throw std::out_of_range("No parents found");
  return tmp->key_;
}
...
void Tree<Key_T, T>::crazyPrinter(const node_* root) const {
  if (root) {
    std::cout << "Node:" << root->key_ << '(' << root->data_ << ')'
              << ":   Parent:";
    if (root->parent_)
      std::cout << root->parent_->key_ << '(' << root->parent_->data_ << ')'
                << ";   Left:";
    else
      std::cout << "NULL"
                << ";   Left:";
    if (root->left_)
      std::cout << root->left_->key_ << '(' << root->left_->data_ << ')'
                << ";   Right:";
    else
      std::cout << "NULL"
                << ";   Right:";
    if (root->right_)
      std::cout << root->right_->key_ << '(' << root->right_->data_ << ')'
                << "." << std::endl;
    else
      std::cout << "NULL"
                << "." << std::endl;
...
typename Tree<Key_T, T>::node_* Tree<Key_T, T>::getBeginig(node_* root) {
  if (root) {
    if (root->left_) return getBeginig(root->left_);
    return root;
  }
  return nullptr;
}
...
typename Tree<Key_T, T>::node_* Tree<Key_T, T>::getEnd(node_* root) {
  if (root) {
    if (root->right_) return getEnd(root->right_);
    return root;
  }
  return nullptr;
}
...
void Tree<Key_T, T>::merge(Tree<Key_T, T>& other) {
  ...
  if (!root_)
    ...
    while (1) {
      ...
      if (pr.second) lst.push(pr.first.getKey());
...
my_containers::vector<std::pair<typename Tree<Key_T, T>::iterator, bool>>
Tree<Key_T, T>::insert_many(Args&&... args) {
  my_containers::vector<std::pair<typename Tree<Key_T, T>::iterator, bool>> res;
