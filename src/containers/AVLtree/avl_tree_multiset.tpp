...
typename Tree<Key_T,T>::iterator Tree<Key_T,T>::insertMult(const_key_type& key, const_value_type& value){
...
template <typename Key_T, typename T>
typename Tree<Key_T,T>::node_* Tree<Key_T,T>::insertMult(node_ *& root,const_key_type& key,const_value_type& value){
    if(!root)
        return root=new node_(key);
    else{
        if(key==root->key_){
            if(root->right_){
                node_* tmp= new node_(root->right_->key_,root->right_->data_,root->right_);
