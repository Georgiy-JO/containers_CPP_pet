...
class list {
 public:
  // Типы, используемые в классе (тип элемента, ссылка на элемент, константная
  // ссылка, размер)
  using value_type = T;
  using reference = T&;
 ...
 private:
  ...
  node* size_node_ =
      nullptr;  // Указатель на вспомогательный узел для хранения размера списка
 ...
 public:
  ...
  class listIterator {
   private:
    ...
    friend class list<value_type>;  // Друзьям класса разрешено доступать к
                                    // приватным данным

   public:
    listIterator();  // Конструктор по умолчанию
    listIterator(node* ptr);  // Конструктор с указателем на узел
    listIterator& operator++();  // Префиксная операция инкремента (перемещение
                                 // на следующий элемент)
    listIterator operator++(int);  // Постфиксная операция инкремента
    listIterator& operator--();  // Префиксная операция декремента (перемещение
                                 // на предыдущий элемент)
    listIterator operator--(int);  // Постфиксная операция декремента
    listIterator operator-(
        size_type value);  // Операция вычитания для перемещения итератора
    ...
    bool operator==(
        listIterator other);  // Операция сравнения итераторов на равенство
    bool operator!=(
        listIterator other);  // Операция сравнения итераторов на неравенство
  };

  // Константный итератор, который расширяет функциональность обычного
  // итератора, запрещая изменение значений
  class listConstIterator : public listIterator {
   public:
    listConstIterator(listIterator other);  // Конструктор, преобразующий
                                            // обычный итератор в константный
    const_reference operator*();  // Разыменование в константном итераторе,
 ...
 public:
  ...
  void set_size_node();  // Устанавливает вспомогательный узел для хранения
                         // размера списка
  list();                // Конструктор по умолчанию
  list(size_type n);     // Конструктор с размером
  list(std::initializer_list<value_type> const&
           items);  // Конструктор для инициализации из списка
  ...
  iterator insert(
      iterator pos,
      const_reference value);  // Вставка элемента в заданную позицию
  ...
  void push_front(
      const_reference value);  // Добавление элемента в начало списка
  ...
  void splice(const_iterator pos,
              list& other);  // Перемещение элементов другого списка в текущий
  ...
  iterator partition(
      iterator first,
      iterator last);  // Разделение списка для быстрой сортировки
  node* merge_nodes(node* left_part,
                    node* right_part);  // Слияние двух отсортированных частей
  void delete_size_node();  // Удаление вспомогательного узла для хранения
                            // размера списка
