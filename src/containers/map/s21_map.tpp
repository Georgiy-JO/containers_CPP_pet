...
template <typename key, typename t>
std::pair<typename map<key, t>::map_iterator, bool> map<key, t>::insert(
...
template <typename key, typename t>
std::pair<const key, t> map<key, t>::map_iterator::operator*() {
  if (avl_tree<key, t>::iterator::iterator_node == nullptr) {
    static std::pair<const key, t> dummy_value{};
    return dummy_value;
  }
  std::pair<const key, t> pr =
      std::make_pair(avl_tree<key, t>::iterator::iterator_node->key_,
                     avl_tree<key, t>::iterator::iterator_node->value_);
  std::pair<const key, t> &ref = pr;
  return ref;
}

template <typename key, typename t>
t &map<key, t>::map_iterator::return_value() {
  if (avl_tree<key, t>::iterator::iterator_node == nullptr) {
    static t dummy_value{};
    return dummy_value;
  }
  return avl_tree<key, t>::iterator::iterator_node->value_;
}

template <typename key, typename t>
typename map<key, t>::map_iterator map<key, t>::find(const key &map_key) {
  typename avl_tree<key, t>::tree_node *node =
      avl_tree<key, t>::find_recursive(avl_tree<key, t>::root_, map_key);
  return map_iterator(node);
}

template <typename key, typename t>
std::pair<typename map<key, t>::map_iterator, bool> map<key, t>::insert_or_assign(
    const key &map_key, const t &obj) {
...
template <typename key, typename t>
template <class... args>
s21::vector<std::pair<typename map<key, t>::map_iterator, bool>>
map<key, t>::insert_many(args &&...args_) {
  s21::vector<std::pair<typename map<key, t>::map_iterator, bool>> vector_;
...
t &map<key, t>::at(const key &map_key) {
  auto map_iterator1 = find(map_key);
  if (map_iterator1 == nullptr)
    throw std::out_of_range(
        "container does not have an element with the specified key");
  return map_iterator1.return_value();
}
...
t &map<key, t>::operator[](const key &map_key) {
  ...
  if (map_iterator1 == nullptr) {
    ...
    map_iterator1 = pr.first;
  }
  return map_iterator1.return_value();
}

template <typename key, typename t>
typename map<key, t>::map_iterator map<key, t>::begin() {
  return map<key, t>::map_iterator(
      avl_tree<key, t>::get_min(avl_tree<key, t>::root_));
}

template <typename key, typename t>
typename map<key, t>::map_iterator map<key, t>::end() {
  ...
  map_iterator test(nullptr, last_node);
  return test;
}

template <typename key, typename t>
typename map<key, t>::const_map_iterator map<key, t>::cbegin() const {
  return map<key, t>::const_map_iterator(
      avl_tree<key, t>::get_min(avl_tree<key, t>::root_));
}

template <typename key, typename t>
typename map<key, t>::const_map_iterator map<key, t>::cend() const {
  if (avl_tree<key, t>::root_ == nullptr) return cbegin();

  typename avl_tree<key, t>::tree_node *last_node =
      avl_tree<key, t>::get_max(avl_tree<key, t>::root_);
  const_map_iterator test(nullptr, last_node);
  return test;
}
...
void map<key, t>::merge(map &other) {
  ...
  map_iterator const_it = const_tree.begin();
  for (; const_it != const_tree.end(); ++const_it) {
    auto map_key = (*const_it).first;
    auto obj = (*const_it).second;
    std::pair<map_iterator, bool> pr = insert(map_key, obj);
    if (pr.second) other.erase(pr.first);
  }
}

template <typename key, typename t>
void map<key, t>::erase(map::map_iterator pos) {
  if (avl_tree<key, t>::root_ == nullptr || pos.iterator_node == nullptr) return;
  avl_tree<key, t>::root_ =
